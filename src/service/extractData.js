...
export const lineChart = (data, interval) => {
  ...
  const startMilliseconds =
    new Date().setHours(0, 0, 0, 0) - interval * 24 * 60 * 60 * 1000;
  if (!data) return [];
  ...
  if (JSON.stringify(data) !== "{}") {
    ...
    for (let i = 0; i < balanceHistory.length; i++) {
      ...
      for (let j = 0; j < accountHistory.length; j++) {
        let dateMilliseconds = Date.parse(
          convertDateInLineChart(accountHistory[j].date)
        );
        ...
        obj.data.push({
          x: date.getTime(),
          y: accountHistory[j].account_balance_cad
        });
        tempMap[formatDate(date)] = {
          x: new Date(
            date.getFullYear(),
            date.getMonth(),
            date.getDate()
...
export const lineChartSingleSeries = (investment_name, data, interval) => {
  ...
  if (JSON.stringify(data) !== "{}") {
    ...
    for (let j = 0; j < balanceHistory.length; j++) {
      let dateMilliseconds = Date.parse(
        convertDateInLineChart(balanceHistory[j].date)
      );
      // if(dateMilliseconds >= startMilliseconds){
      //obj.data.push( {x:Date.parse(convertDateInLineChart(balanceHistory[j].date)), y:balanceHistory[j].account_balance/* _cad */} )
      obj.data.push({
        x: new Date(balanceHistory[j].date).getTime(),
        y: parseFloat(balanceHistory[j].account_balance) /* _cad */
      });
...
export const formatRatesHistoryData = data => {
  ...
  chartData = currency_rate_histories.map(rate_history => {
    ...
    obj["data"] = rate_history.rates.map(rate => {
      return {
        x: Date.parse(convertDateInLineChart(rate.date)),
...
export const getMinimumY = chartData => {
  ...
  chartData.forEach(series => {
    series.data.forEach(data => {
      ...
      if (data.y != 0) y_values.push(data.y);
    });
  });
