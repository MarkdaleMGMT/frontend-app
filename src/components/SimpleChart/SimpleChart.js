...
import {
  ...
  formatUserHistoryData,
  formatRatesHistoryData,
  getMinimumY
} from "../../service/extractData";
...
export default class SimpleChart extends Component {
  ...
  extractChartData(data, interval, dataType) {
    if (dataType === "users") return formatUserHistoryData("Users", data);
  ...
  getChartOptions(chartData, chartType, dataType, interval, color_index) {
    ...
    if (dataType == "users") tooltip = { ...tooltip, valueSuffix: " users" };
        ...
        <div className="simple-chart-wrapper">
          <div className="simple-chart-controls">
            <div style={{ display: "inline-flex" }}>
              <div onClick={this.handleClickLineChart} className="chart-title">
                {chartTitle}
              </div>
            </div>
            <div>
