export function formatAmount(amount, isCAD = false) {
  ...
  let reverseBeforeDecimal = beforeDecimal
    .split("")
    .reverse()
    .join("");
  beforeDecimal = reverseBeforeDecimal
    .replace(/.{3}/g, "$& ,")
    .split("")
    .reverse()
    .join("");
  ...
  else afterDecimal = afterDecimal.replace(/.{4}/g, "$&, ");
...
export function filterRow(row, query) {
  ...
  if (query[0] === "$") {
    ...
    return String(row.amount_cad).includes(query);
  } else {
    return (
      new Date(row.time).toLocaleDateString().includes(query) ||
      String(row.amount_cad).includes(query) ||
      String(row.amount).includes(query) ||
      String(row.account_balance).includes(query) ||
      row.description.toLowerCase().includes(query) ||
      row.investment_name.toLowerCase().includes(query)
    );
  }
